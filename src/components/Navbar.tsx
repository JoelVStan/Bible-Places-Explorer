      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-6">
        <div className="flex items-center justify-between h-14 sm:h-16 gap-1.5 sm:gap-3">
          ...
          <div
            ...
            className="flex items-center gap-2 cursor-pointer shrink-0 group select-none min-w-0"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-amber-600 to-amber-800 text-amber-50 flex items-center justify-center shadow-md shadow-amber-900/20 group-hover:scale-105 transition-transform shrink-0">
              <Compass className="w-4 h-4 sm:w-5 sm:h-5 text-amber-200 animate-spin-slow" />
            </div>
            <div className="min-w-0 truncate">
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold text-xs sm:text-base md:text-lg tracking-tight text-stone-900 dark:text-stone-50 group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors truncate">
              ...
              <p className="text-[10px] sm:text-[11px] text-amber-800 dark:text-amber-400 font-medium -mt-0.5 font-scripture truncate">
          ...
          <div className="relative flex-1 max-w-xs md:max-w-sm hidden md:block">
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-xs text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 p-0.5 cursor-pointer"
                title="Clear search"
              >
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className={`md:hidden p-2 rounded-xl text-stone-600 dark:text-stone-300 transition-colors cursor-pointer active:scale-95 ${
            ...
            <button
              onClick={onOpenDaily}
              className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-full text-xs font-semibold bg-gradient-to-r from-amber-500/15 to-orange-500/15 hover:from-amber-500/25 hover:to-orange-500/25 border border-amber-300 dark:border-amber-700/60 text-amber-900 dark:text-amber-200 transition-all shadow-xs shrink-0 cursor-pointer active:scale-95"
              title="Today's Biblical Place"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
              <span className="hidden sm:inline">Today's Place</span>
              <span className="sm:hidden text-[11px] font-bold">ഇന്ന്</span>
            </button>

            {/* Bookmarks Button */}
            <button
              onClick={onOpenBookmarks}
              className="relative p-2 rounded-xl hover:bg-stone-200/60 dark:hover:bg-stone-800 text-stone-700 dark:text-stone-300 transition-colors shrink-0 cursor-pointer active:scale-95"
              title="Saved Places"
        ...
        {mobileSearchOpen && (
          <div className="pb-2.5 md:hidden animate-in fade-in slide-in-from-top-2">
        ...
        <nav className="hidden md:flex items-center justify-center gap-1.5 py-1.5 border-t border-amber-200/40 dark:border-stone-800/80 overflow-x-auto no-scrollbar">
          <button
            onClick={() => onTabChange('map')}
