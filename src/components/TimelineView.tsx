      ...
      <div className="relative pl-6 sm:pl-8 space-y-6 before:absolute before:left-3 sm:before:left-4 before:top-3 before:bottom-3 before:w-1 before:bg-gradient-to-b before:from-amber-400 before:via-amber-600 before:to-orange-500 rounded-full">
        {filteredEvents.map((item) => (
          <div key={item.id} className="relative group">
            ...
            {/* Event Card */}
            <div className="p-5 rounded-3xl bg-white dark:bg-stone-800/90 border border-stone-200 dark:border-stone-700/80 shadow-md hover:shadow-xl hover:border-amber-400 transition-all flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="space-y-2 max-w-2xl">
                ...
                <h3 className="text-base sm:text-lg font-bold text-stone-900 dark:text-stone-50 group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors flex items-baseline gap-2">
              ...
              <div className="flex items-center gap-2 shrink-0 pt-2 md:pt-0 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-stone-100 dark:border-stone-700">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-amber-50 dark:bg-stone-700 hover:bg-amber-100 dark:hover:bg-stone-600 text-amber-900 dark:text-amber-200 text-xs font-semibold flex items-center gap-1.5 transition-colors"
                >
                ...
                <button
                  onClick={() => onSelectPlaceId(item.placeId)}
                  className="px-3 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-colors"
                >
